...
本アプリケーションはKosuzuki2025と共同で設計しました。このREADMEの最後に自身の担当内容を記載します。
...
* **本アプリケーションにおけるItsuki-Shimizu-3939の設計担当箇所**  
  * カーリルAPIを用いた蔵書検索システムの実装
  * 検索したい市区町村について視覚的に分かりやすいダイアログの実装
  * ローディング画面の実装
  * その他レイアウトの調整

* **工夫した点**\
　私は、本アプリケーションにおいて主にカーリルAPIを用いた蔵書検索システムの実装を行いました。  
　まず、検索対象の書籍を特定するために楽天ブックスAPIを用いて対象書籍のISBNを取得し、それをカーリルAPIに受け渡すことで図書館毎のその書籍の蔵書状況を取得できるシステムを構築しました。この際、異なるAPI間でデータを連携させる設計を行うことで、ユーザーが意識せずとも書籍の選択から蔵書状況の確認までを一気に行えるようにしました。  
　また、カーリルAPIでの蔵書状況の取得の際に、確認先の図書館のシステムによっては一度のAPIの呼び出しでは情報を取得しきれず、不完全な状態で処理が終了してしまうという問題がありました。そこで、カーリルAPIのレスポンスに含まれるcontinueフラグ(検索処理がまだ継続しているかを示すフラグ)に着目し、データ取得が完了するまで再取得を行うポーリング機能を追加することによってこの問題を解決し、安定した蔵書状況の取得を実現しました。  
　さらに、図書館によっては検索に時間がかかるため、ユーザーの視点からは検索の終了タイミングがわからないという課題がありました。この問題に対しては検索中であることが一目でわかるようなローディング画面を実装することでユーザビリティの向上を図りました。
　加えて、タブにマウスを重ねた際に視覚的なフィードバックが与えられるようなホバーの実装も行いました。

* **困難だったポイント**

　カーリルAPIで検索結果を受け取る際に、json形式のデータの扱い方に慣れておらず、必要な情報の取得・整理に苦労しました。特に、図書館名と蔵書状況を同時に取得する際に仮想配列についての理解が甘く、つまずいてしまいました。  
　また、ローディング画面の設計についても、CSSのpositionについての理解ができておらず、自分の配置した位置になかなか配置できず苦戦しました。  
　今回の設計で躓いたポイントについては今後さらに勉強していきたいと思います。
